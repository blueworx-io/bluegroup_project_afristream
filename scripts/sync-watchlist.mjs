...
# Do not edit by hand; re-run the sync instead. One entry per line as
# "<tt-id> <imdb-rating>", the rating omitted when IMDb has none yet; lines
...
# resolving each id through TMDB for artwork and using the IMDb rating for the
# card badge and the rating filter. Only the first N (see the read-side cap)
# are used.
#
...
async function main() {
  ...
  try {
    ...
    console.log(
      `Wrote ${entries.length} title IDs (${rated} with an IMDb rating)` +
        `${total ? ` of ${total} in the watchlist` : ''} to data/editor-picks-ids.txt`
    );
